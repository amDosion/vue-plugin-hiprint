/**
 * format.ts — date / number formatters (ported from V2).
 *
 * V1: hinnn.dateFormat / hinnn.numFormat (bundle.js line 324-360).
 * V3: 与 V2 一致, TS 类型化 + 严格类型. Behavior 与 V1 100% 一致.
 *
 ...
 *  - S    millisecond
 */
export function dateFormat(value: unknown, pattern: string): string {
  if (!value) return ''
  try {
    const o = new Date('' + (value as string | number))
    ...
    for (const i in map) {
      if (new RegExp('(' + i + ')').test(result)) {
        ...
        result = result.replace(
          RegExp.$1,
          RegExp.$1.length === 1 ? String(v) : ('00' + v).substr(('' + v).length)
        )
      }
    }
    return result
 ...
 *
 * @param value      Number-like value (string or number)
 * @param precision  Decimal places (0 / undefined → integer)
 * @returns          Formatted string (toFixed when precision > 0), integer otherwise, or '' on error
 */
export function numFormat(
  ...
  try {
    ...
    const l = parseInt(String(precision))
