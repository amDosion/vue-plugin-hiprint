/**
 * group-by.spec.ts — groupBy / orderBy.
 */
import { describe, it, expect } from 'vitest'
import { groupBy, orderBy } from '../group-by'
...
describe('groupBy', () => {
  ...
  it('empty array → empty result', () => {
    expect(groupBy([], ['x'], (r: Record<string, unknown>) => r.x)).toEqual([])
  })
...
describe('orderBy', () => {
  ...
  it('empty / single-element array', () => {
    expect(orderBy([], (x: number) => x)).toEqual([])
