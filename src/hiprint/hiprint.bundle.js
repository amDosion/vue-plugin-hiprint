...
var hiprint = function (t) {
  ...
  n.d(e, "a", function () {
    ...
    if (t) try {
      ...
      return console.error('[hiprint] format failed:', t), "";
    }
    return "";
    ...
    return Object.defineProperty(t, "instance", {
      ...
      if (-1 == $(t).attr("src").indexOf("base64")) try {
        ...
        try {
          ...
          console.error('[hiprint] formatter/eval caught:', t);
        }
      }
    ...
    BasePrintElement = function () {
      ...
      return BasePrintElement.prototype.getConfigOptionsByName = function (t) {
        ...
        this.designTarget.hidraggable({
          // 添加 draggable 属性（同时考虑位置锁定）
          draggable: n.options.positionLocked ? false : n.options.draggable,
        ...
        // 初始化时应用锁定状态
        var _initPosLocked = !!this.options.positionLocked;
        var _initSizeLocked = !!this.options.sizeLocked;
        if (_initPosLocked || _initSizeLocked) {
          var _$initRp = this.designTarget.find('.resize-panel');
          if (_$initRp.length) {
            if (_initPosLocked) {
              _$initRp.addClass('locked');
              if (!_$initRp.find('.hiprint-lock-badge').length) _$initRp.append('<div class="hiprint-lock-badge">🔒</div>');
              _$initRp.find('.del-btn').hide();
            }
            if (_initSizeLocked || _initPosLocked) {
              _$initRp.find('.resizebtn').hide();
            }
          } else {
            // noContainer 元素
            if (_initSizeLocked || _initPosLocked) {
        ...
        var els = this.panel.printElements.filter(function (t) {
          ...
          // 同步 draggable 字段，使所有依赖 options.draggable 的代码正确工作
          if (_posLocked) {
            this.options.draggable = false;
          } else {
            // 解锁时恢复 draggable（删除该字段，让默认行为生效）
          ...
          if (_$rp && _$rp.length) {
            if (_posLocked) {
              _$rp.addClass('locked');
              if (!_$rp.find('.hiprint-lock-badge').length) _$rp.append('<div class="hiprint-lock-badge">🔒</div>');
            } else {
              _$rp.removeClass('locked');
              _$rp.find('.hiprint-lock-badge').remove();
            }
            // 大小锁定：隐藏调整圆点
            if (_sizeLocked || _posLocked) {
              _$rp.find('.resizebtn').hide();
            } else {
              _$rp.find('.resizebtn').show();
            }
            // 位置锁定时隐藏删除按钮
            if (_posLocked) {
              _$rp.find('.del-btn').hide();
            } else {
              _$rp.find('.del-btn').show();
            }
          }
          // noContainer 元素（如表格）
          if (!_$rp || !_$rp.length) {
            if (_sizeLocked || _posLocked) {
              ...
              this.designTarget.find('.resizebtn').show();
              this.designTarget.removeClass('size-locked');
            }
            if (_posLocked) {
              this.designTarget.addClass('position-locked');
            } else {
              this.designTarget.removeClass('position-locked');
            }
          }
          // 位置锁定时同步锁定宽高大小面板
          var _self = this;
          ...
          if (o && o.tabs && o.tabs.length) {
            this.getPrintElementOptionTabs().forEach(function (tab) {
              tab.list.forEach(function (e) {
                if (e.name === 'widthHeight') _whItem = e;
              });
            });
          } else {
            this.getPrintElementOptionItems().forEach(function (e) {
          ...
          if (_whItem) {
            _whItem.setValue(_self.options, _self);
          }
          hinnn.event.trigger("hiprintTemplateDataChanged_" + this.templateId, "元素修改");
        ...
        this.designTarget.hireizeable({
          ...
          draggable: n.options.positionLocked ? false : n.options.draggable, // 元素是否可拖拽、删除
          // 是否显示宽高box
        ...
        try {
          ...
          console.warn('[hiprint] copyJson success');
        } catch (e) {
          flag = false;
          console.error('[hiprint] copyJson failed:', e);
        }
        return flag;
        ...
        if (false === this.options.draggable || this.options.positionLocked) return;
  ...
  var i = function () {
    ...
    b = function () {
      ...
      return t.prototype.createTarget = function (t) {
        ...
        if (t && t.length) {
          var c = t.find('.hiprint-printElement-content');
          if (!c.length) c = t;
          if (e) return c.css("border-width", e + "pt"), "border-width:" + e + "pt";
          c[0].style.borderWidth = "";
        }
    ...
    coordinate = function () {
      ...
      return t.prototype.createTarget = function (t, o) {
        ...
        // 锁定位置切换
        n.target.find('.coord-pos-lock').on('change', function () {
          n.posLocked = $(this).is(':checked');
          n.target.find('input[type=number]').prop('disabled', n.posLocked);
          n.target.find('.auto-submit:first').trigger('change');
        });
        return n.target;
        ...
        n.target.find("input[type=number]:first").after(n.lockTarget);
        // 同步编辑...
        n.target.find("input[type=number]:first").change(function () {
          if (n.syncLock) {
            n.target.find("input[type=number]:last").val($(this).val())
          }
        });
        n.target.find("input[type=number]:last").change(function () {
          if (n.syncLock) {
            n.target.find("input[type=number]:first").val($(this).val())
          }
        });
        return n.lockTarget
        ...
        var v = {
          ...
          positionLocked: this.posLocked,
          left: 0,
          top: 0,
        }
        v.left = parseFloat(this.target.find("input[type=number]:first").val() || 0)
        v.top = parseFloat(this.target.find("input[type=number]:last").val() || 0)
        return v;
      }, t.prototype.setValue = function (t, el) {
        this.el = el.designTarget || el;
        this.target.find("input[type=number]:first").val(t.left);
        this.target.find("input[type=number]:last").val(t.top);
        this.posLocked = !!(t.positionLocked);
        this.target.find('.coord-pos-lock').prop('checked', this.posLocked);
        this.target.find('input[type=number]').prop('disabled', this.posLocked);
      }, t.prototype.destroy = function () {
        this.target.remove();
      }, t;
    }(),
    widthHeight = function () {
      ...
      return t.prototype.createTarget = function (t, o) {
        ...
        n.createSyncLock(n.syncLock);
        // 锁定大小切换
        n.target.find('.wh-size-lock').on('change', function () {
          n.sizeLocked = $(this).is(':checked');
          n.ownSizeLocked = n.sizeLocked; // 用户主动切换时更新自身状态
          n.target.find('input[type=number]').prop('disabled', n.sizeLocked);
          n.target.find('.auto-submit:first').trigger('change');
        });
        return n.target;
      }, t.prototype.createSyncLock = function (t) {
        ...
        var v = {
          ...
          sizeLocked: this.ownSizeLocked, // 返回自身的锁定状态
          width: 0,
          height: 0,
        }
        v.width = parseFloat(this.target.find("input[type=number]:first").val() || 0)
        v.height = parseFloat(this.target.find("input[type=number]:last").val() || 0)
        return v;
      }, t.prototype.setValue = function (t, el) {
        this.el = el.designTarget || el;
        this.target.find("input[type=number]:first").val(t.width);
        this.target.find("input[type=number]:last").val(t.height);
        // 保存自身的锁定状态
        this.ownSizeLocked = !!(t.sizeLocked);
        // 显示状态：位置锁定时强制锁定大小
        var effectiveLocked = !!(t.sizeLocked || t.positionLocked);
        this.sizeLocked = effectiveLocked;
        this.target.find('.wh-size-lock').prop('checked', effectiveLocked);
        this.target.find('input[type=number]').prop('disabled', effectiveLocked);
        // 位置锁定时，大小锁定不可取消
        if (t.positionLocked) {
          this.target.find('.wh-size-lock').prop('disabled', true);
        } else {
          this.target.find('.wh-size-lock').prop('disabled', false);
        }
      }, t.prototype.destroy = function () {
        this.target.remove();
      }, t;
    ...
    C = function () {
      ...
      return t.prototype.createTarget = function (t) {
        this.el = t;
        var e = void 0, i = this;
        this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__('图片地址')}\n        </div>\n        <div class="hiprint-option-item-field" style="display: flex;align-items: center;flex-wrap: wrap;gap:4px;">\n        <input type="text" placeholder="${i18n.__('请输入图片地址')}" class="auto-submit" style="flex:1;min-width:0">\n    <button class="hiprint-option-item-settingBtn" style="padding:0 10px;margin:0" type="button">${i18n.__('选择')}</button>\n    <label class="hiprint-option-item-settingBtn" style="padding:0 10px;margin:0;cursor:pointer">${i18n.__('上传')}<input type="file" accept="image/*" style="display:none" class="hiprint-img-upload"></label>\n        </div>\n    </div>`);
        ...
        // 本地图片上传 → base64
        this.target.find('.hiprint-img-upload').on('change', function () {
        ...
        var t = this.target.find("input[type=text]").val();
        if (t) return t.toString();
      }, t.prototype.setValue = function (t) {
    ...
    O = function () {
      ...
      return t.prototype.css = function (t, e) {
        if (t && t.length) {
          var c = t.find('.hiprint-printElement-content');
          if (!c.length) c = t;
          if (e) return c.css("border-color", e), "border-color:" + e;
          c[0].style.borderColor = "";
        }
    ...
    watermarkOptions = function () {
      ...
      return t.prototype.createTarget = function () {
        ...
        this.fieldsWrapper.append(this.content);
        this.fieldsWrapper.append(this.fillStyle);
        this.fieldsWrapper.append(this.fontSize);
        this.fieldsWrapper.append(this.rotate);
        this.fieldsWrapper.append(this.width);
        this.fieldsWrapper.append(this.height);
        this.fieldsWrapper.append(this.timestamp);
        this.fieldsWrapper.append(this.format);
        ...
        if (!this.target.find('.watermark-toggle').is(':checked')) {
          return {};
        }
        let opt = {
        ...
        this.options = t;
        var hasWatermark = t && t.content;
        this.target.find('.watermark-toggle').prop('checked', !!hasWatermark);
        this.fieldsWrapper.css('display', hasWatermark ? '' : 'none');
    ...
    gridOptions = function () {
      ...
      return t.prototype.createTarget = function () {
        ...
        this.target.append(this.fieldsWrapper);
        this.target.find('.grid-toggle').on('change', function () {
          if ($(this).is(':checked')) {
            self.fieldsWrapper.slideDown(150);
          } else {
            self.fieldsWrapper.slideUp(150);
        ...
        if (!this.target.find('.grid-toggle').is(':checked')) {
          return {};
        }
        return {
          enabled: true,
          size: parseFloat(this.gridSize.find('input').val()) || 5,
          color: this.gridColor.find('input').val() || "rgba(0,0,0,0.1)"
        };
      }, t.prototype.setValue = function (t) {
        var enabled = t && t.enabled;
        this.target.find('.grid-toggle').prop('checked', !!enabled);
        this.fieldsWrapper.css('display', enabled ? '' : 'none');
        this.gridSize.find('input').val(t && t.size != null ? t.size : 5);
        this.gridColor.find('input').val(t && t.color || "rgba(0,0,0,0.1)");
        this.gridColor.find('input').minicolors({
          format: "rgb",
          opacity: true,
          theme: "bootstrap"
        });
    ...
    printMarginOptions = function () {
      function t() {
        this.name = "printMarginOptions";
      }
      return t.prototype.createTarget = function () {
        var self = this;
        ...
        this.fieldsWrapper.append(this.marginRight);
        this.fieldsWrapper.append(this.marginColor);
        this.target.append(this.fieldsWrapper);
        this.target.find('.margin-toggle').on('change', function () {
          if ($(this).is(':checked')) {
            self.fieldsWrapper.slideDown(150);
          } else {
            self.fieldsWrapper.slideUp(150);
        ...
        if (!this.target.find('.margin-toggle').is(':checked')) {
          return {};
        }
        return {
          enabled: true,
          top: parseFloat(this.marginTop.find('input').val()) || 0,
          bottom: parseFloat(this.marginBottom.find('input').val()) || 0,
          left: parseFloat(this.marginLeft.find('input').val()) || 0,
          right: parseFloat(this.marginRight.find('input').val()) || 0,
          color: this.marginColor.find('input').val() || "rgba(87,179,255,0.5)"
        };
      }, t.prototype.setValue = function (t) {
        var enabled = t && t.enabled;
        this.target.find('.margin-toggle').prop('checked', !!enabled);
        this.fieldsWrapper.css('display', enabled ? '' : 'none');
        this.marginTop.find('input').val(t && t.top != null ? t.top : 15);
        this.marginBottom.find('input').val(t && t.bottom != null ? t.bottom : 15);
        this.marginLeft.find('input').val(t && t.left != null ? t.left : 15);
        this.marginRight.find('input').val(t && t.right != null ? t.right : 15);
        this.marginColor.find('input').val(t && t.color || "rgba(87,179,255,0.5)");
        this.marginColor.find('input').minicolors({
          format: "rgb",
          opacity: true,
          theme: "bootstrap"
        });
    ...
    B = function () {
      ...
      return t.prototype.createTarget = function () {
        return this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__('首页页尾')}\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" step="1" placeholder="${i18n.__('首页页尾')}" class="auto-submit">\n        </div>\n    </div>`), this.target;
      }, t.prototype.getValue = function () {
        var t = this.target.find("input").val();
        if (t) return parseFloat(t.toString());
      }, t.prototype.setValue = function (t) {
    ...
    F = function () {
      ...
      return t.prototype.createTarget = function () {
        return this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__('尾页页尾')}\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" step="1" placeholder="${i18n.__('尾页页尾')}" class="auto-submit">\n        </div>\n    </div>`), this.target;
      }, t.prototype.getValue = function () {
        var t = this.target.find("input").val();
        if (t) return parseFloat(t.toString());
      }, t.prototype.setValue = function (t) {
    ...
    L = function () {
      ...
      return t.prototype.createTarget = function () {
        return this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__('偶数页页尾')}\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" step="1" placeholder="${i18n.__('偶数页页尾')}" class="auto-submit">\n        </div>\n    </div>`), this.target;
      }, t.prototype.getValue = function () {
        var t = this.target.find("input").val();
        if (t) return parseFloat(t.toString());
      }, t.prototype.setValue = function (t) {
    ...
    A = function () {
      ...
      return t.prototype.createTarget = function () {
        return this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__('奇数页页尾')}\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" step="1" placeholder="${i18n.__('奇数页页尾')}" class="auto-submit" >\n        </div>\n    </div>`), this.target;
      }, t.prototype.getValue = function () {
        var t = this.target.find("input").val();
        if (t) return parseFloat(t.toString());
      }, t.prototype.setValue = function (t) {
    ...
    N = function () {
      ...
      return t.prototype.createTarget = function () {
        return this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__('左偏移')}\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" step="1" placeholder="${i18n.__('偏移量')}pt" class="auto-submit" >\n        </div>\n    </div>`), this.target;
      }, t.prototype.getValue = function () {
        var t = this.target.find("input").val();
        if (t) return parseFloat(t.toString());
      }, t.prototype.setValue = function (t) {
    ...
    G = function () {
      ...
      return t.prototype.css = function (t, e) {
        if (t && t.length) {
          var c = t.find('.hiprint-printElement-content');
          if (!c.length) c = t;
          if (e) return c.css("border-top-style", e), "border-top:1px";
          c[0].style.borderTopStyle = "", c[0].style.borderTopWidth = "";
        }
    ...
    q = function () {
      ...
      return t.prototype.css = function (t, e) {
        if (t && t.length) {
          var c = t.find('.hiprint-printElement-content');
          if (!c.length) c = t;
          if (e) return c.css("border-left-style", e), "border-left:1px";
          c[0].style.borderLeftStyle = "", c[0].style.borderLeftWidth = "";
        }
    ...
    X = function () {
      ...
      return t.prototype.css = function (t, e) {
        if (t && t.length) {
          var c = t.find('.hiprint-printElement-content');
          if (!c.length) c = t;
          if (e) return c.css("border-right-style", e), "border-right:1px";
          c[0].style.borderRightStyle = "", c[0].style.borderRightWidth = "";
        }
    ...
    Y = function () {
      ...
      return t.prototype.css = function (t, e) {
        if (t && t.length) {
          var c = t.find('.hiprint-printElement-content');
          if (!c.length) c = t;
          if (e) return c.css("border-bottom-style", e), "border-bottom-style:1px solid";
          c[0].style.borderBottomStyle = "", c[0].style.borderBottomWidth = "";
        }
    ...
    TablePrintElement = function (_super) {
      ...
      return __extends(TablePrintElement, _super), TablePrintElement.prototype.getColumns = function () {
        ...
        this.designTarget.hidraggable({
          ...
          draggable: n.options.positionLocked ? false : undefined,
          axis: n.options.axis ? n.options.axis : void 0,
          designTarget: n,
          onDrag: function onDrag(t, i, o) {
            n.updateSizeAndPositionOptions(i, o), n.createLineOfPosition(e);
        ...
        // 初始化表格锁定状态
        var _tblPosLocked = !!this.options.positionLocked;
        var _tblSizeLocked = !!this.options.sizeLocked;
        if (_tblPosLocked || _tblSizeLocked) {
          if (_tblSizeLocked || _tblPosLocked) {
            this.designTarget.find('.resizebtn').hide();
            this.designTarget.addClass('size-locked');
          }
          if (_tblPosLocked) {
  ...
  n = jQuery, i = {
      ...
      n(t).each(function () {
        ...
        var r = {
          ...
          h = function h() {
            ...
            return n.each([r, a, p, s, l, u, d, c, rotateBtn], function (e, o) {
    ...
    bindResizeEvent: function bindResizeEvent(t, e) {
      ...
      t.on("mousedown", ".w", function (e) {
        ...
        s = i.options.noContainer ? u[0].offsetLeft : u.offset().left;
      });
      var f = !1; // 上
      t.on("mousedown", ".n", function (e) {
        r = e.pageY, p = t.height(), f = !0;
        l = i.options.noContainer ? u[0].offsetTop : u.offset().top;
      });
      var g = !1; // 右上
      ...
      t.on("mousedown", function (t) {
        ...
        if (!d && !c && !h && !f && !g && !m && !v && !y && !rt) {
          o = t.pageX, r = t.pageY, l = u.offset().top, s = u.offset().left;
        }
        b = !1;
  ...
  jQuery, n = "connected", i = "reconnecting", window.hiwebSocket = {
    ...
    send: function send(t) {
      try {
        ...
        console.error('[hiprint] socket send data failed (' + (t || '') + '):', e);
      }
    },
    sendByFragments: function(content) {
      try {
        ...
        if (!contentToSplit || !this.socket) return;
        // 字符总数
        ...
        console.error('[hiprint] socket send fragment failed (' + (content || '') + '):', e);
      }
    },
    ...
    refreshPrinterList: function refreshPrinterList() {
      try {
        ...
        console.error('[hiprint] refreshPrinterList failed:', e);
      }
    },
    getPaperSizeInfo: function getPaperSizeInfo(printer) {
      try {
        ...
        console.error('[hiprint] getPaperSizeInfo failed:', e)
      }
    },
    getClients: function getClients() {
      try {
        ...
        console.error('[hiprint] getClients failed:', e);
      }
    },
    getClientInfo: function getClientInfo() {
      try {
        ...
        console.error('[hiprint] getClientInfo failed:', e)
      }
    },
    getAddress: function getAddress(type, ...args) {
      try {
        ...
        console.error('[hiprint] getAddress failed:', e);
      }
    },
    ippPrint: function ippPrint(options) {
      try {
        ...
        console.error('[hiprint] ippPrint failed:', e);
      }
    },
    ippRequest: function ippRequest(options) {
      try {
        ...
        console.error('[hiprint] ippRequest failed:', e);
      }
    },
    ...
    start: function start(cb) {
      ...
      this.socket = window.io(this.host, {
        ...
        t.opened = !0, console.warn('[hiprint] Websocket opened.'),
        ...
        }), _this.socket.off("clients").on("clients", function(clients) {
          t.clients = clients;
          hinnn.event.trigger("clients", clients)
        }), _this.socket.off("clientInfo").on("clientInfo", function(clientInfo) {
          t.clientInfo = clientInfo
          hinnn.event.trigger("clientInfo", clientInfo)
        }), _this.socket.off("printerList").on("printerList", function (e) {
          t.printerList = e;
          hinnn.event.trigger("printerList", e);
        }), _this.socket.off("paperSizeInfo").on("paperSizeInfo", function(e) {
          t.paperSize = Array.isArray(e)?e:[e]
          hinnn.event.trigger("paperSizeInfo", t.paperSize);
        }), _this.socket.off("address").on("address", function (type, addr, e) {
          hinnn.event.trigger("address_" + type, {'addr': addr, 'e': e});
        }), _this.socket.off("ippPrinterConnected").on("ippPrinterConnected", function (printer) {
          hinnn.event.trigger("ippPrinterConnected", printer);
        }), _this.socket.off("ippPrinterCallback").on("ippPrinterCallback", function (err, res) {
          hinnn.event.trigger("ippPrinterCallback", {'err': err, 'res': res});
        }), _this.socket.off("ippRequestCallback").on("ippRequestCallback", function (err, res) {
          hinnn.event.trigger("ippRequestCallback", {'err': err, 'res': res});
        }), t.state = n;
        cb && cb(true, e);
      }), this.socket.on("connect_error", function (e) {
        t.opened = !1;
        console.error('[hiprint] socket connect_error:', e);
    ...
    reconnect: function reconnect() {
      this.state !== n && this.state !== i || (this.stop(), this.ensureReconnectingState() && (console.warn('[hiprint] Websocket reconnecting.'), this.start()));
    },
    stop: function stop() {
      this.socket && (console.warn('[hiprint] Closing the Websocket.'), this.socket.close(), this.socket = null, this.printerList = []);
    },
  ...
  var _r,
    ...
    p = n(1),
    s = n(2),
    isIconifyIcon = function (icon) {
      return !!icon && /^[a-z0-9]+:[a-z0-9-]+$/i.test(icon);
    },
    escapeHtmlAttr = function (val) {
      return String(null == val ? "" : val).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    },
    legacyGlyphiconToIconify = {
      "glyphicon-text-width": "ep:document",
      "glyphicon-picture": "ep:picture",
      "glyphicon-subscript": "ep:tickets",
      "glyphicon-th": "ep:grid",
      "glyphicon-header": "ep:postcard",
      "glyphicon-minus": "ep:minus",
      "glyphicon-link": "ep:link",
      "glyphicon-yen": "ep:money",
      "glyphicon-tag": "ep:price-tag",
      "glyphicon-send": "ep:promotion",
      "glyphicon-user": "ep:user",
      "glyphicon-resize-horizontal": "ep:minus",
      "glyphicon-resize-vertical": "ep:more-filled",
      "glyphicon-unchecked": "ep:crop",
      "glyphicon-record": "ep:aim",
      "glyphicon-barcode": "ep:list",
      "glyphicon-qrcode": "ep:grid",
      "glyphicon-chevron-left": "ep:arrow-left",
      "glyphicon-chevron-right": "ep:arrow-right"
    },
    normalizeIconName = function (icon) {
      if (!icon || "string" != typeof icon) return "";
      var iconValue = $.trim(icon);
      if (!iconValue) return "";
      if (isIconifyIcon(iconValue)) return iconValue;
      if (legacyGlyphiconToIconify[iconValue]) return legacyGlyphiconToIconify[iconValue];
      var parts = iconValue.split(/\s+/);
      for (var i = 0; i < parts.length; i++) {
        if (legacyGlyphiconToIconify[parts[i]]) return legacyGlyphiconToIconify[parts[i]];
      }
      return "";
    },
    iconifyFallbackText = function (icon) {
      var map = {
        "ep:document": "T",
        "ep:picture": "IMG",
        "ep:tickets": "TXT",
    ...
    renderIconHtml = function (icon, fallbackText) {
      if (!icon || "string" != typeof icon) return "";
      var iconValue = $.trim(icon);
      if (!iconValue) return "";
      if ("<" === iconValue.charAt(0)) return iconValue;
      var normalized = normalizeIconName(iconValue);
      if (normalized) {
        var fallback = null == fallbackText ? iconifyFallbackText(normalized) : fallbackText;
        return '<iconify-icon class="hiprint-icon hiprint-iconify" icon="' + escapeHtmlAttr(normalized) + '">' + escapeHtmlAttr(fallback) + "</iconify-icon>";
      }
      return '<span class="hiprint-icon hiprint-icon-fallback">' + escapeHtmlAttr(iconValue) + '</span>';
    },
    setToggleIcon = function ($toggle, icon, fallbackText) {
      var $icon = $toggle.find(".hiprint-icon").first();
      if (!$icon.length) return;
      if ($icon.is("iconify-icon") || $icon.hasClass("hiprint-iconify")) {
        var normalized = normalizeIconName(icon);
        $icon.attr("icon", normalized || "");
        $icon.text(null == fallbackText ? iconifyFallbackText(normalized) : fallbackText);
      } else {
        $icon.text(null == fallbackText ? iconifyFallbackText(normalizeIconName(icon)) : fallbackText);
      }
    },
    l = function () {
      function t() {
      ...
      return t.prototype.createPrintElementTypeHtml = function (t, e) {
        var i = $(t),
          n = $('<div class="hiprint-ep-type-container"></div>');
        return e.forEach(function (t) {
          ...
          t.printElementTypes && t.printElementTypes.length ? t.printElementTypes.forEach(function (t) {
            var iconHtml = renderIconHtml(t.icon);
          ...
          n.append(group);
        }), i.append(n), n.find(".ep-draggable-item");
      }, t;
    }(),
    u = n(5),
    d = n(15),
    ...
    h = function () {
      function t(t) {
        var e = this;
        this.text = t.text, this.field = t.field, this.fields = t.fields, this.title = t.title, this.tid = t.tid, this.data = t.data, this.styler = t.styler, this.formatter = t.formatter, this.type = t.type, this.options = t.options, this.icon = t.icon, this.editable = t.editable != void 0 ? t.editable : !0, this.columnDisplayEditable = t.columnDisplayEditable != void 0 ? t.columnDisplayEditable : !0, this.columnDisplayIndexEditable = t.columnDisplayIndexEditable != void 0 ? t.columnDisplayIndexEditable : !0, this.columnTitleEditable = t.columnTitleEditable != void 0 ? t.columnTitleEditable : !0, this.columnResizable = t.columnResizable != void 0 ? t.columnResizable : !0, this.columnAlignEditable = t.columnAlignEditable != void 0 ? t.columnAlignEditable : !0, this.columns = [], (t.columns || []).forEach(function (t, n) {
    ...
    T = function () {
      ...
      return t.prototype.subscribePaperBaseInfoChanged = function (t) {
        ...
        var e = this;
        this.createHeaderLine(), this.createFooterLine(), this.target.addClass("design"), this.applyGridOptions(this.gridOptions), this.paperNumberTarget = this.createPaperNumber(this.formatPaperNumber(1, 1), true), this.createRuler(), this.createWaterMark(true, this.panelIdx, this.watermarkOptions), this.createPrintMargins(this.printMarginOptions), this.resetPaperNumber(this.paperNumberTarget), $(this.paperNumberTarget).bind("dblclick.hiprint", function () {
      ...
      }, t.prototype.createPrintMargins = function (opts) {
        this.destroyPrintMargins();
        if (!opts || !opts.enabled) return;
        var mt = (opts.top != null ? opts.top : 15) + 'pt';
        var mb = (opts.bottom != null ? opts.bottom : 15) + 'pt';
        var ml = (opts.left != null ? opts.left : 15) + 'pt';
        var mr = (opts.right != null ? opts.right : 15) + 'pt';
        var color = opts.color || 'rgba(87,179,255,0.5)';
        var base = 'position:absolute;z-index:0;pointer-events:none;background:' + color + ';';
        this._printMarginTop = $('<div class="hiprint-print-margin hiprint-print-margin-top" style="' + base + 'top:0;left:0;right:0;height:' + mt + ';"></div>');
        this._printMarginBottom = $('<div class="hiprint-print-margin hiprint-print-margin-bottom" style="' + base + 'bottom:0;left:0;right:0;height:' + mb + ';"></div>');
        this._printMarginLeft = $('<div class="hiprint-print-margin hiprint-print-margin-left" style="' + base + 'top:' + mt + ';bottom:' + mb + ';left:0;width:' + ml + ';"></div>');
        this._printMarginRight = $('<div class="hiprint-print-margin hiprint-print-margin-right" style="' + base + 'top:' + mt + ';bottom:' + mb + ';right:0;width:' + mr + ';"></div>');
        this.target.append(this._printMarginTop);
        this.target.append(this._printMarginBottom);
        this.target.append(this._printMarginLeft);
        this.target.append(this._printMarginRight);
      }, t.prototype.destroyPrintMargins = function () {
        this.target.find('.hiprint-print-margin').remove();
        this._printMarginTop = null;
        this._printMarginBottom = null;
        this._printMarginLeft = null;
        this._printMarginRight = null;
      }, t.prototype.applyGridOptions = function (opts) {
        if (opts && opts.enabled) {
          this.target.addClass("grid");
          var size = (opts.size || 5) + "mm";
          var color = opts.color || "rgba(0,0,0,0.1)";
          this.target.css({
            "background-image": "linear-gradient(90deg, " + color + " 3%, transparent 3%), linear-gradient(360deg, " + color + " 3%, transparent 3%)",
            "background-size": size + " " + size,
            "background-position": "left top"
          });
        } else {
          this.target.removeClass("grid");
          this.target.css({
            "background-image": "",
            "background-size": "",
            "background-position": ""
    ...
    barcode = function(t) {
      ...
      return N(e, t), e.prototype.updateDesignViewFromOptions = function () {
        ...
        try {
          ...
          console.error('[hiprint] barcode render failed:', error)
    ...
    j = function () {
      function t(t) {
        this.field = t.field, this.fields = t.fields, this.title = t.title, this.text = t.text, this.tid = t.tid, this.data = t.data, this.styler = t.styler, this.formatter = t.formatter, this.type = t.type, this.onRendered = t.onRendered, this.options = t.options, this.icon = t.icon;
      }
      ...
      return t.prototype.getText = function (t) {
        ...
        var e = {};
        return $.extend(e, this.getOptions() || {}, t || {}), W.createPrintElement(this, e);
      }, t.prototype.getPrintElementTypeEntity = function () {
        return new c({
          title: this.title,
          type: this.type
      ...
      return t.prototype.design = function (t) {
        ...
        this.orderPrintElements(), this.designPaper = this.createNewPage(0), this.target.html(""), this.target.append(this.designPaper.getTarget()), this.droppablePaper(this.designPaper), this.designPaper.design(t), this.designPaper.subscribePaperBaseInfoChanged(function (t) {
          ...
          let panelOptions = {
            ...
            printMarginOptions: e.printMarginOptions || {},
          ...
          o.a.event.trigger("BuildCustomOptionSettingEventKey_" + e.templateId, {
            ...
            callback: function callback(t) {
              ...
              e.printMarginOptions = t.printMarginOptions || void 0; if (t.printMarginOptions && t.printMarginOptions.enabled) { e.designPaper.createPrintMargins(t.printMarginOptions); } else { e.designPaper.destroyPrintMargins(); }
        ...
        try {
          ...
          objList.forEach((obj,index) => {
            ...
            console.warn('[hiprint] pasteJson success');
          ...
          console.error('[hiprint] pasteJson failed:', e);
        }
      }, t.prototype.css = function (t) {
        ...
        return n.setFooter(this.firstPaperFooter, this.evenPaperFooter, this.oddPaperFooter, this.lastPaperFooter), n.setOffset(this.leftOffset, this.topOffset), n.printMarginOptions = this.printMarginOptions, n.gridOptions = this.gridOptions, n;
        ...
        // 设计区域屏蔽浏览器右键，绑定自定义右键菜单
        var panel = t;
        t.designPaper.getTarget().on("contextmenu", function (e) {
          e.preventDefault();
          e.stopPropagation();
          ...
          menu.append(groupTitle2);
          // 字体大小
          var fontSizeItem = $('<div class="hiprint-ctx-menu-item">' + i18n.__('字体') + ' 12pt</div>');
          fontSizeItem.on("click", function () {
            $(".hiprint-ctx-menu").remove();
            selectedEls.forEach(function (el) { el.updateOption('fontSize', 12, true); });
            o.a.event.trigger("hiprintTemplateDataChanged_" + panel.templateId, "批量修改");
          });
          menu.append(fontSizeItem);
          // 字体加粗
          var fontWeightItem = $('<div class="hiprint-ctx-menu-item">' + i18n.__('字体加粗') + '</div>');
          fontWeightItem.on("click", function () {
            $(".hiprint-ctx-menu").remove();
            selectedEls.forEach(function (el) { el.updateOption('fontWeight', 'bolder', true); });
            o.a.event.trigger("hiprintTemplateDataChanged_" + panel.templateId, "批量修改");
          });
          ...
          deleteItem.on("click", function () {
            $(".hiprint-ctx-menu").remove();
            selectedEls.forEach(function (el) {
              el.designTarget && el.designTarget.length && el.designTarget.remove();
              var idx = panel.printElements.indexOf(el);
              if (idx > -1) panel.printElements.splice(idx, 1);
            });
            if (panel.mouseRect && panel.mouseRect.target) {
              panel.mouseRect.target.remove();
              panel.mouseRect = null;
            }
        ...
        if (this.mouseRect && this.mouseRect.mouseRectSelectedElement && this.mouseRect.mouseRectSelectedElement.length) {
          return this.mouseRect.mouseRectSelectedElement;
        }
        return this.printElements.filter(function (t) {
          return t.designTarget && 'block' == t.designTarget.children().last().css('display')
            && t.designTarget.children().last().hasClass('selected')
            && !t.printElementType.type.includes('table');
        });
        ...
        var bounds = els.map(function (el) {
          return { el: el, left: el.options.getLeft(), top: el.options.getTop(), width: el.options.getWidth(), height: el.options.getHeight() };
        });
        var minLeft = Math.min.apply(null, bounds.map(function(b){ return b.left }));
        var maxRight = Math.max.apply(null, bounds.map(function(b){ return b.left + b.width }));
        var minTop = Math.min.apply(null, bounds.map(function(b){ return b.top }));
        var maxBottom = Math.max.apply(null, bounds.map(function(b){ return b.top + b.height }));
        switch (type) {
          case 'left':
            bounds.forEach(function(b){ b.el.updateSizeAndPositionOptions(minLeft, b.top) }); break;
          case 'right':
            bounds.forEach(function(b){ b.el.updateSizeAndPositionOptions(maxRight - b.width, b.top) }); break;
          case 'top':
            bounds.forEach(function(b){ b.el.updateSizeAndPositionOptions(b.left, minTop) }); break;
          case 'bottom':
            bounds.forEach(function(b){ b.el.updateSizeAndPositionOptions(b.left, maxBottom - b.height) }); break;
          case 'horizontalCenter':
            var cx = (minLeft + maxRight) / 2;
            bounds.forEach(function(b){ b.el.updateSizeAndPositionOptions(cx - b.width / 2, b.top) }); break;
          case 'verticalCenter':
            var cy = (minTop + maxBottom) / 2;
            bounds.forEach(function(b){ b.el.updateSizeAndPositionOptions(b.left, cy - b.height / 2) }); break;
          case 'distributeHorizontal':
            if (els.length < 3) return;
            bounds.sort(function(a, b){ return a.left - b.left });
            var totalW = bounds.reduce(function(s, b){ return s + b.width }, 0);
            var gapH = (maxRight - minLeft - totalW) / (bounds.length - 1);
            var cursorH = bounds[0].left + bounds[0].width + gapH;
            for (var i = 1; i < bounds.length - 1; i++) {
              bounds[i].el.updateSizeAndPositionOptions(cursorH, bounds[i].top);
              cursorH += bounds[i].width + gapH;
            }
            break;
          case 'distributeVertical':
            if (els.length < 3) return;
            bounds.sort(function(a, b){ return a.top - b.top });
            var totalH = bounds.reduce(function(s, b){ return s + b.height }, 0);
            var gapV = (maxBottom - minTop - totalH) / (bounds.length - 1);
            var cursorV = bounds[0].top + bounds[0].height + gapV;
            for (var j = 1; j < bounds.length - 1; j++) {
              bounds[j].el.updateSizeAndPositionOptions(bounds[j].left, cursorV);
              cursorV += bounds[j].height + gapV;
            }
            break;
        }
        ...
        hinnn.event.trigger("hiprintTemplateDataChanged_" + this.templateId, "对齐");
      }, t.prototype.createElementListPanel = function () {
        var panel = this;
        // 切换按钮
        var toggleBtn = $('<div class="hiprint-el-list-toggle" title="' + i18n.__('元素列表') + '">☰</div>');
        // 列表面板
        ...
        // 监听元素选中事件，同步高亮列表行
        o.a.event.on("PrintElementSelectEventKey_" + panel.templateId, function (data) {
          if (!panel._elListPanel || !panel._elListPanel.hasClass("visible")) return;
          var selectedEl = data && data.printElement;
          if (!selectedEl) return;
          panel._elListBody.find(".hiprint-el-list-row").each(function (idx) {
            var row = $(this);
            if (idx < panel.printElements.length && panel.printElements[idx] === selectedEl) {
              row.addClass("selected-el");
              // 滚动到可见区域
              var container = panel._elListBody[0];
              var rowEl = row[0];
              if (rowEl.offsetTop < container.scrollTop || rowEl.offsetTop + rowEl.offsetHeight > container.scrollTop + container.clientHeight) {
                rowEl.scrollIntoView({ block: "nearest" });
              }
            } else {
              row.removeClass("selected-el");
            }
          });
        });
      }, t.prototype.refreshElementList = function () {
        var panel = this;
      ...
      return t.prototype.design = function (t, e) {
        ...
        if (-1 == e.indexOf("base64")) try {
          ...
          try {
            ...
            console.error('[hiprint] xhrLoadImage caught:', t);
          }
        }
      }, t.prototype.xhrLoadImage = function (t) {
  ...
  function buildToolbar(container, template, options) {
    ...
    var opts = $.extend({
      ...
      businessButtonText: i18n.__('业务选择'),
      onBusinessClick: null,
      onBusinessDialogOpen: null,
      onBusinessDialogClose: null,
      businessDialogTitle: i18n.__('选择业务'),
      businessDialogEmptyText: i18n.__('暂无业务'),
      businessDialogLoadingText: i18n.__('业务加载中...'),
      businessDialogErrorText: i18n.__('业务加载失败'),
      businessListProvider: null,
      businessLoader: null,
      onBusinessSelect: null,
      closeBusinessDialogOnSelect: true,
      onTemplateDialogOpen: null,
      onTemplateDialogClose: null,
      ...
      customPaperButtonText: i18n.__('自定义'),
      customPaperConfirmText: i18n.__('确定'),
      rotateButtonText: i18n.__('旋转'),
      previewButtonText: i18n.__('预览'),
      clearButtonText: i18n.__('清空'),
      printButtonText: i18n.__('打印'),
    ...
    var toolbarApi = null;
    var toolbarButtonRegistry = {};
    var toolbarGroupRegistry = {};
    var businessItems = [];
    var businessLoading = false;
    var businessError = '';
    var businessDialogRequestId = 0;
    ...
    var $saveDialog = null;
    var dialogHandlerKeyMap = {
      businessOpen: 'onBusinessDialogOpen',
      businessClose: 'onBusinessDialogClose',
      templateOpen: 'onTemplateDialogOpen',
      templateClose: 'onTemplateDialogClose',
      saveOpen: 'onSaveDialogOpen',
      saveClose: 'onSaveDialogClose',
      templateDeleteConfirm: 'onTemplateDeleteConfirm'
    };

    function getDialogHandler(handlerKey) {
      var optionKey = dialogHandlerKeyMap[handlerKey] || handlerKey;
      return opts[optionKey];
    }

    function setDialogHandler(handlerKey, handler) {
      var optionKey = dialogHandlerKeyMap[handlerKey] || handlerKey;
      opts[optionKey] = handler;
      return opts[optionKey];
    }
    ...
    function normalizeBusinessItem(item, index) {
      if (!item) return null;
      var name = item.name || item.title || item.businessName || item.label || (i18n.__('未命名业务') + ' ' + (index + 1));
      var id = item.id || item.businessId || item.key || item.code || ('business_' + index);
      return $.extend({}, item, {
        _idx: index,
        _bid: id,
        _name: name
    ...
    function registerToolbarGroup(key, $group) {
      if (!key || !$group || !$group.length) return $group;
      toolbarGroupRegistry[key] = $group;
      return $group;
    }

    function registerToolbarButton(key, $button, meta) {
      if (!key || !$button || !$button.length) return $button;
      toolbarButtonRegistry[key] = $.extend({
        $el: $button,
        groupKey: null
      }, meta || {});
      return $button;
    ...
    function getToolbarButtonRecord(key) {
      return key && toolbarButtonRegistry[key] ? toolbarButtonRegistry[key] : null;
    }

    function getToolbarButton(key) {
      var record = getToolbarButtonRecord(key);
      return record ? record.$el : null;
    }

    function syncToolbarGroupVisibility(groupKey) {
      if (!groupKey) return;
      var $group = toolbarGroupRegistry[groupKey];
      if (!$group || !$group.length) return;
      var hasVisibleButton = false;
      $.each(toolbarButtonRegistry, function (btnKey, record) {
        if (record && record.groupKey === groupKey && record.$el && record.$el.length && record.$el.css('display') !== 'none') {
          hasVisibleButton = true;
          return false;
        }
      });
      $group.toggle(hasVisibleButton);
    }

    function setToolbarButtonVisible(key, visible) {
      var record = getToolbarButtonRecord(key);
      if (!record || !record.$el || !record.$el.length) return false;
      record.$el.toggle(visible !== false);
      syncToolbarGroupVisibility(record.groupKey);
      return true;
    }

    function setToolbarButtonDisabled(key, disabled) {
      var record = getToolbarButtonRecord(key);
      if (!record || !record.$el || !record.$el.length) return false;
      record.$el.prop('disabled', !!disabled);
      return true;
    }
    ...
    function triggerToolbarButton(key) {
      var record = getToolbarButtonRecord(key);
      if (!record || !record.$el || !record.$el.length) return false;
      record.$el.trigger('click');
      return true;
    }

    function setToolbarGroupVisible(groupKey, visible) {
      var $group = toolbarGroupRegistry[groupKey];
      if (!$group || !$group.length) return false;
      $group.toggle(visible !== false);
      return true;
    }

    function getToolbarButtons() {
      var map = {};
      $.each(toolbarButtonRegistry, function (key, record) {
    ...
    function getToolbarGroups() {
      var map = {};
      $.each(toolbarGroupRegistry, function (key, $group) {
        if (!$group) return;
        map[key] = $group;
      });
      return map;
    ...
    function downloadTemplateJson(json, filename) {
      try {
        ...
        console.error('[hiprint] downloadJson failed:', e);
      }
    }

    function renderBusinessDialog() {
      if (!$businessDialog || !$businessDialog.length) return;
      $businessDialog.find('.hiprint-toolbar-template-title').text(opts.businessDialogTitle || i18n.__('选择业务'));
      var $body = $businessDialog.find('.hiprint-toolbar-template-body');
      $body.empty();
      if (businessLoading) {
        $body.append('<div class="hiprint-toolbar-template-state loading">' + (opts.businessDialogLoadingText || i18n.__('业务加载中...')) + '</div>');
        return;
      }
      if (businessError) {
        $body.append('<div class="hiprint-toolbar-template-state error">' + businessError + '</div>');
        return;
      }
      if (!businessItems.length) {
        $body.append('<div class="hiprint-toolbar-template-state empty">' + (opts.businessDialogEmptyText || i18n.__('暂无业务')) + '</div>');
        return;
      }
    ...
    function refreshBusinessList() {
      if (typeof opts.businessListProvider !== 'function') {
        businessLoading = false;
        businessError = '';
        renderBusinessDialog();
        return Promise.resolve((businessItems || []).slice());
      }
      var reqId = ++businessDialogRequestId;
      businessLoading = true;
      businessError = '';
      renderBusinessDialog();
      return Promise.resolve(opts.businessListProvider(template, toolbarApi)).then(function (list) {
        if (reqId !== businessDialogRequestId) return businessItems;
        if (!Array.isArray(list)) list = [];
        businessItems = list.map(function (item, index) {
          return normalizeBusinessItem(item, index);
        }).filter(function (item) {
          return !!item;
        });
        businessError = '';
        ...
        if (reqId !== businessDialogRequestId) return businessItems;
        businessError = (err && err.message) || opts.businessDialogErrorText || i18n.__('业务加载失败');
        return businessItems;
      }).finally(function () {
        if (reqId !== businessDialogRequestId) return;
        businessLoading = false;
        renderBusinessDialog();
      });
    }

    function closeBusinessDialogDefault() {
      if ($businessDialog && $businessDialog.length) {
        $businessDialog.hide();
      }
    }

    function closeBusinessDialog(payload) {
      var context = {
        type: 'business',
        payload: payload,
        template: template,
    ...
    function resolveBusinessData(item) {
      if (typeof opts.businessLoader === 'function') {
        return Promise.resolve(opts.businessLoader(item, template, toolbarApi));
      }
      return Promise.resolve(item && (item.businessConfig || item.fieldsConfig || item.config || item.data || null));
    }

    function handleBusinessSelect(item) {
      ...
      return resolveBusinessData(item).then(function (rawData) {
        ...
        console.error('[hiprint] businessLoader rejected:', err);
      });
    }
    ...
    function ensureBusinessDialog() {
      ...
      $businessDialog.on('click', '.hiprint-toolbar-business-refresh', function () {
        refreshBusinessList();
      });
      $businessDialog.on('click', '.hiprint-toolbar-business-action', function () {
        var idx = parseInt($(this).attr('data-index'));
        if (isNaN(idx) || !businessItems[idx]) return;
        var action = $(this).attr('data-action');
        var item = businessItems[idx];
        if (action === 'select') {
    ...
    function openBusinessDialogDefault() {
      ensureBusinessDialog();
      $businessDialog.show();
      renderBusinessDialog();
      refreshBusinessList();
    }

    function openBusinessDialog(payload) {
      var context = {
        type: 'business',
        payload: payload,
        template: template,
        api: toolbarApi,
        openDefault: openBusinessDialogDefault,
        closeDefault: closeBusinessDialogDefault
      };
    ...
    function closeTemplateDialogDefault() {
      if ($templateDialog && $templateDialog.length) {
        $templateDialog.hide();
      }
    }

    function closeTemplateDialog(payload) {
      var context = {
        type: 'template',
        payload: payload,
        template: template,
    ...
    function confirmTemplateDeleteDefault(item) {
      return confirm(i18n.__('是否确认删除') + '?');
    }

    function confirmTemplateDelete(item) {
      if (typeof opts.onTemplateDeleteConfirm === 'function') {
        var context = {
          type: 'templateDelete',
          item: item,
          template: template,
      ...
      return Promise.resolve(confirmTemplateDeleteDefault(item));
    }
    ...
    function openTemplateDialogDefault() {
    ...
    function openTemplateDialog(payload) {
      var context = {
        type: 'template',
        payload: payload,
        template: template,
        api: toolbarApi,
        openDefault: openTemplateDialogDefault,
        closeDefault: closeTemplateDialogDefault
      };
    ...
    function closeSaveDialogDefault() {
      if ($saveDialog && $saveDialog.length) {
        $saveDialog.hide();
      }
    }

    function closeSaveDialog(payload) {
      var context = {
        type: 'save',
        payload: payload,
        template: template,
    ...
    function openSaveDialogDefault(defaultName) {
      ensureSaveDialog();
      var $input = $saveDialog.find('.hiprint-toolbar-save-input');
      var $error = $saveDialog.find('.hiprint-toolbar-save-error');
      $error.hide();
    ...
    function openSaveDialog(defaultName, payload) {
      var context = {
        type: 'save',
        payload: payload,
        ...
        template: template,
        api: toolbarApi,
        openDefault: function (name) {
          openSaveDialogDefault(name == null ? defaultName : name);
        },
        closeDefault: closeSaveDialogDefault
      };
    ...
    function triggerSave(payload) {
      ...
      openSaveDialog(payload && payload.name, payload);
      return null;
    }
    ...
    if (opts.showBusinessSelect) {
      ...
      $businessSelectGroup.append($businessSelectBtn);
      $toolbar.append($businessSelectGroup);
    }
    ...
    if (opts.showPaperSelect) {
      var $paperGroup = registerToolbarGroup('paper', $('<div class="hiprint-toolbar-group hiprint-toolbar-paper"></div>'));
    ...
    // --- 缩放 ---
    if (opts.showScale) {
      var $scaleGroup = registerToolbarGroup('scale', $('<div class="hiprint-toolbar-group hiprint-toolbar-scale"></div>'));
    ...
    var createExtraButton = function (btnOpt, api) {
      ...
      if (btnOpt.key) {
        registerToolbarButton('extra:' + btnOpt.key, $btn, { groupKey: 'extra' });
      }
    ...
    toolbarApi = {
      ...
      getBusinessDialogElement: function () {
        return ensureBusinessDialog();
      },
      openBusinessDialog: function () {
        openBusinessDialog();
      },
      closeBusinessDialog: function () {
        closeBusinessDialog();
      },
      refreshBusinessList: function () {
        return refreshBusinessList();
      },
      ...
      getBusinessItems: function () {
        return (businessItems || []).slice();
      },
      setBusinessListProvider: function (provider) {
        opts.businessListProvider = provider;
      },
      setBusinessLoader: function (loader) {
        opts.businessLoader = loader;
      },
      getTemplateDialogElement: function () {
        return ensureTemplateDialog();
      },
      ...
      getSaveDialogElement: function () {
        return ensureSaveDialog();
      },
      ...
      setDialogHandler: function (handlerKey, handler) {
        return setDialogHandler(handlerKey, handler);
      },
      getDialogHandler: function (handlerKey) {
        return getDialogHandler(handlerKey);
      },
      setBusinessDialogOpenHandler: function (handler) {
        return setDialogHandler('businessOpen', handler);
      },
      setBusinessDialogCloseHandler: function (handler) {
        return setDialogHandler('businessClose', handler);
      },
      setTemplateDialogOpenHandler: function (handler) {
        return setDialogHandler('templateOpen', handler);
      },
      setTemplateDialogCloseHandler: function (handler) {
        return setDialogHandler('templateClose', handler);
      },
      setSaveDialogOpenHandler: function (handler) {
        return setDialogHandler('saveOpen', handler);
      },
      setSaveDialogCloseHandler: function (handler) {
        return setDialogHandler('saveClose', handler);
      },
      setTemplateDeleteConfirmHandler: function (handler) {
        return setDialogHandler('templateDeleteConfirm', handler);
      },
      getButton: function (key) {
        return getToolbarButton(key);
      },
      getButtons: function () {
        return getToolbarButtons();
      },
      setButtonVisible: function (key, visible) {
        return setToolbarButtonVisible(key, visible);
      },
      setButtonDisabled: function (key, disabled) {
        return setToolbarButtonDisabled(key, disabled);
      },
      setButtonText: function (key, text, useHtml) {
        return setToolbarButtonText(key, text, useHtml);
      },
      triggerButton: function (key) {
        return triggerToolbarButton(key);
      },
      getGroup: function (groupKey) {
        return toolbarGroupRegistry[groupKey] || null;
      },
      getGroups: function () {
        return getToolbarGroups();
      },
      setGroupVisible: function (groupKey, visible) {
        return setToolbarGroupVisible(groupKey, visible);
      }
    };

    // --- 扩展按钮（配置式） ---
    if (Array.isArray(opts.extraButtons) && opts.extraButtons.length) {
      var $extraGroup = registerToolbarGroup('extra', $('<div class="hiprint-toolbar-group hiprint-toolbar-extra"></div>'));
      opts.extraButtons.forEach(function (btnOpt, index) {
        if (btnOpt && !btnOpt.key) {
          btnOpt.key = '' + index;
        }
    ...
    toolbarCtrl = {
      ...
      setBusinessItems: function (list) {
        toolbarApi.setBusinessItems(list);
      },
      getBusinessItems: function () {
        return toolbarApi.getBusinessItems();
      },
      setBusinessListProvider: function (provider) {
        toolbarApi.setBusinessListProvider(provider);
      },
      setBusinessLoader: function (loader) {
        toolbarApi.setBusinessLoader(loader);
      },
      getBusinessDialogElement: function () {
        return toolbarApi.getBusinessDialogElement();
      },
      openTemplateDialog: function () {
        openTemplateDialog();
      },
      ...
      getTemplateDialogElement: function () {
        return toolbarApi.getTemplateDialogElement();
      },
      openSaveDialog: function (defaultName) {
        openSaveDialog(defaultName);
      },
      ...
      getSaveDialogElement: function () {
        return toolbarApi.getSaveDialogElement();
      },
      refreshTemplateList: function () {
        return refreshTemplateList();
      },
      ...
      setDialogHandler: function (handlerKey, handler) {
        return toolbarApi.setDialogHandler(handlerKey, handler);
      },
      getDialogHandler: function (handlerKey) {
        return toolbarApi.getDialogHandler(handlerKey);
      },
      setBusinessDialogOpenHandler: function (handler) {
        return toolbarApi.setBusinessDialogOpenHandler(handler);
      },
      setBusinessDialogCloseHandler: function (handler) {
        return toolbarApi.setBusinessDialogCloseHandler(handler);
      },
      setTemplateDialogOpenHandler: function (handler) {
        return toolbarApi.setTemplateDialogOpenHandler(handler);
      },
      setTemplateDialogCloseHandler: function (handler) {
        return toolbarApi.setTemplateDialogCloseHandler(handler);
      },
      setSaveDialogOpenHandler: function (handler) {
        return toolbarApi.setSaveDialogOpenHandler(handler);
      },
      setSaveDialogCloseHandler: function (handler) {
        return toolbarApi.setSaveDialogCloseHandler(handler);
      },
      setTemplateDeleteConfirmHandler: function (handler) {
        return toolbarApi.setTemplateDeleteConfirmHandler(handler);
      },
      triggerSave: function (payload) {
        return triggerSave(payload);
      },
      getButton: function (key) {
        return toolbarApi.getButton(key);
      },
      getButtons: function () {
        return toolbarApi.getButtons();
      },
      setButtonVisible: function (key, visible) {
        return toolbarApi.setButtonVisible(key, visible);
      },
      setButtonDisabled: function (key, disabled) {
        return toolbarApi.setButtonDisabled(key, disabled);
      },
      setButtonText: function (key, text, useHtml) {
        return toolbarApi.setButtonText(key, text, useHtml);
      },
      triggerButton: function (key) {
        return toolbarApi.triggerButton(key);
      },
      getGroup: function (groupKey) {
        return toolbarApi.getGroup(groupKey);
      },
      getGroups: function () {
        return toolbarApi.getGroups();
      },
      setGroupVisible: function (groupKey, visible) {
        return toolbarApi.setGroupVisible(groupKey, visible);
      },
  ...
  function buildDesigner(container, options) {
    ...
    var $leftToggle = $('<div class="hiprint-designer-edge-toggle hiprint-designer-edge-toggle-left" title="' + i18n.__('折叠组件栏') + '">' +
      renderIconHtml('ep:arrow-left', '<') + '</div>');
    ...
    var $rightToggle = $('<div class="hiprint-designer-edge-toggle hiprint-designer-edge-toggle-right" title="' + i18n.__('折叠属性栏') + '">' +
      renderIconHtml('ep:arrow-right', '>') + '</div>');
    ...
    function applyLeftState() {
      ...
      if (leftCollapsed) {
        ...
        setToggleIcon($leftToggle, "ep:arrow-right", ">");
      } else {
        $leftResizeHandle.show();
        $leftToggle.attr('title', i18n.__('折叠组件栏'));
        setToggleIcon($leftToggle, "ep:arrow-left", "<");
      }
    }
    ...
    function applyRightState() {
      ...
      if (rightCollapsed) {
        ...
        setToggleIcon($rightToggle, "ep:arrow-left", "<");
      } else {
        $rightResizeHandle.show();
        $rightToggle.attr('title', i18n.__('折叠属性栏'));
        setToggleIcon($rightToggle, "ep:arrow-right", ">");
      }
    }
