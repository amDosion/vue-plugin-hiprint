...
插件内置了纯 jQuery 实现的工具栏，包含模版选择、纸张选择、缩放、旋转、对齐、预览、清空、打印、保存，不依赖任何 UI 框架。点击“保存”会先弹出输入框，要求填写模版名称后再执行保存回调。
...
const toolbarCtrl = hiprint.buildToolbar('#hiprintToolbar', hiprintTemplate, {
  ...
  onPrint: (template) => {
    // 自定义打印逻辑（默认走浏览器打印，不需要客户端）
    template.print(printData)
  },
  onClear: (template) => {
    // 自定义清空逻辑，不传则使用内置 confirm 确认后清空
  },
...
5. 工具栏：选择模版、纸张、自定义纸张、缩放、旋转、对齐、预览、清空、打印、保存。
...
#hiprintDesigner {
  ...
  height: 100%;
  overflow: hidden;
  padding: 10px; /* 需要保留时必须配合 box-sizing */
}
...
1. `src/demo/*` 是演示页面与示例数据，不是插件核心依赖。
2. 第三方项目不需要复制 `src/demo`，只需要使用插件导出的 `hiprint` 能力。
3. 当前 demo 的“默认空模板”来自演示逻辑（`sessionStorage.hiprintEmptyTemplateOnStart`），不是插件强制行为。

#### 11.12.2 想保持一致，必须对齐这 4 层

1. 初始化层：`hiprint.init({ providers, lang })` 与当前项目一致。
2. 设计器层：使用 `hiprint.buildDesigner(...)`，不要自行拼接三栏 DOM。
3. 工具栏层：对齐 `toolbarOptions`（模板中心、保存、纸张、缩放、旋转、对齐、预览、清空、打印）。
...
1. 误把 `src/demo/design/index.vue` 的业务逻辑当插件运行时逻辑。
